Feature: Performing operations on wish list
  ...
  @addingAndRemovingProductToWishList
  Scenario Outline: Add and remove product from wish list
    Given I am on the store home page
    When I click store button
    And I click on "<sportType>"
    And I open specific "<product>"
    ...
    And I go to the wish list
    Then I will be able to see added "<product>"
    When I remove "<product>" from wish list
    Then I will be able to see "<message>" about deleted product

    Examples:
      | sportType   | product          | message                  |
      | Windsurfing | Egipt – El Gouna | Produkt został usunięty. |
